...
export default function AdminPanel() {
  ...
  useEffect(() => { if (!isModerator) navigate('/', { replace: true }); }, [isModerator, navigate]);
  ...
  const loadUsers = async (page = uPage) => {
    ...
    setULoading(true);
    try {
      const data = await adminApi.listUsers({ query: uQuery, role: uRole, active: uActive, page, limit: uLimit });
      setUsers(data.items || []);
  ...
  useEffect(() => { if (tab === 'tournaments') loadTournaments(1); }, [tab]);
  ...
  const toggleActive = async (u) => {
    try {
      ...
      loadUsers(uPage);
    } catch (e) { toast.error(e.message || 'Nie udało się zmienić statusu'); }
  };
  ...
  const changeRole = async (u, role) => {
    try {
      ...
      loadUsers(uPage);
    } catch (e) { toast.error(e.message || 'Nie udało się zmienić roli'); }
  };
  ...
  const hardDeleteTournament = async (t) => {
    ...
    } catch (e) { toast.error(e.message || 'Nie udało się usunąć turnieju'); }
  };
  ...
  const crumbs = [{ label: 'Home', href: '/' }, { label: 'Administracja', href: '/admin' }, { label: tab === 'users' ? 'Użytkownicy' : 'Turnieje' }];

  const renderPagination = (page, totalPages, go) => (
    <div className="ap-pagination" aria-label="Paginacja">
      <button onClick={() => go(1)} disabled={page === 1} aria-label="Pierwsza">⏮</button>
      <button onClick={() => go(Math.max(1, page - 1))} disabled={page === 1} aria-label="Poprzednia">‹</button>
      <span>Strona {page} / {totalPages}</span>
      <button onClick={() => go(Math.min(totalPages, page + 1))} disabled={page === totalPages} aria-label="Następna">›</button>
      <button onClick={() => go(totalPages)} disabled={page === totalPages} aria-label="Ostatnia">⏭</button>
    </div>
  );
  ...
  return (
    <section className="container ap-wrap">
      ...
      <div className="ap-tabs">
        {isAdmin && (
          <button className={`st-tab ${tab === 'users' ? 'active' : ''}`} onClick={() => setTab('users')}>
            Użytkownicy
          </button>
        )}
        {isModerator && (
          <button className={`st-tab ${tab === 'tournaments' ? 'active' : ''}`} onClick={() => setTab('tournaments')}>
            Turnieje
          </button>
        )}
      </div>
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div className="ap-filters">
              <input className="input" placeholder="Szukaj (email, imię, nazwisko)…" value={uQuery} onChange={e => setUQuery(e.target.value)} />
              ...
              <select className="input" value={uActive} onChange={e => setUActive(e.target.value)}>
                <option value="">— aktywne? —</option>
                <option value="true">aktywne</option>
                <option value="false">dezaktywowane</option>
              </select>
              <button className="btn-primary" onClick={onFilterUsers} disabled={uLoading}>Filtruj</button>
            </div>
                ...
                <div className="tbl-wrap">
                  <table className="tbl" role="table">
                    ...
                    <tbody>
                      {users.map(u => {
                        const curRole =
                          (u.user_roles?.[0]?.roles?.role_name || u.user_roles?.[0]?.role || 'user').toLowerCase();
                        return (
                          <tr key={u.id}>
                            ...
                            <td>{u.active ? <span className="badge ok">tak</span> : <span className="badge off">nie</span>}</td>
                            <td>
                              <span className={
                                curRole === 'admin' ? 'badge role-admin' :
                                curRole === 'moderator' ? 'badge role-moderator' : 'badge role-user'
                              }>
                                {curRole}
                              </span>
                            </td>
                            <td className="actions">
                              ...
                              <select className="input input-sm" value={curRole} onChange={e => changeRole(u, e.target.value)}>
                                <option value="user">user</option>
                                <option value="moderator">moderator</option>
                                <option value="admin">admin</option>
                              </select>
      ...
      {tab === 'tournaments' && isModerator && (
        <div className="card">
          ...
          <div className="card-body">
            <div className="ap-filters">
              <input className="input" placeholder="Szukaj po nazwie…" value={tQuery} onChange={e => setTQuery(e.target.value)} />
              <button className="btn-primary" onClick={onFilterTournaments} disabled={tLoading}>Filtruj</button>
            </div>
                    ...
                    <tbody>
                      {tournaments.map(t => (
                        <tr key={t.id}>
                          ...
                          <td>{t.name}</td>
                          <td>{t.organizer ? `${t.organizer.name} ${t.organizer.surname}` : '—'}</td>
                          <td>{t.city || '—'}</td>
                          <td className="nowrap">
                          ...
                          <td>
                            <div className="status-cell">
                              <span className={`status-dot ${t.status}`}></span> {t.status || '—'}
                              {t.applicationsOpen === false && <div className="muted small">zapisy wstrzymane</div>}
                            </div>
                          </td>
                          <td className="actions">
                            <button className="btn-secondary btn-sm" onClick={() => hideTournament(t, t.status !== 'hidden')}>
                              {t.status === 'hidden' ? 'Pokaż' : 'Ukryj'}
                            </button>
                            <button className="btn-secondary btn-sm" onClick={() => toggleApplications(t)}>
                              {t.applicationsOpen ? 'Wstrzymaj zapisy' : 'Otwórz zapisy'}
                            </button>
                            <button className="btn-warning btn-sm" onClick={() => softDelete(t)}>
                              Oznacz jako usunięty
                            </button>
                            <button className="btn-delete btn-sm" onClick={() => hardDeleteTournament(t)}>
                              Usuń (twardo)
                            </button>
                          </td>
                        </tr>
